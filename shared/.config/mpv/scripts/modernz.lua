					...
					if mouse_hit(element) or force_seek_tooltip then
						...
						elseif element.thumbnailable and not thumbfast.disabled then
							...
							if osd_w then
								...
								if hover_dur then
									hover_sec = hover_dur * sliderpos / 100
								end
...
local function osc_init()
	...
	ne.eventresponder["mbtn_left_up"] = function()
		mp.commandv(
			...
			"set",
			"video-zoom",
			math.max(user_opts.zoom_out_min, mp.get_property_number("video-zoom", 0) - 0.05)
		)
	end
	...
	ne.eventresponder["wheel_up_press"] = function()
		mp.commandv(
			...
			"set",
			"video-zoom",
			math.min(user_opts.zoom_in_max, mp.get_property_number("video-zoom", 0) + 0.05)
		)
	end
	...
	ne.tooltip_style = osc_styles.tooltip
	ne.tooltipF = function()
		return user_opts.tooltip_hints and (state.file_loop and locale.file_loop_enable or locale.file_loop_disable)
			or ""
	end
	ne.eventresponder["mbtn_left_up"] = function()
	...
	ne.tooltip_style = osc_styles.tooltip
	ne.tooltipF = function()
		return user_opts.tooltip_hints and (state.shuffled and locale.shuffle or locale.unshuffle) or ""
	end
	ne.eventresponder["mbtn_left_up"] = function()
...
local function render()
	...
	local function run_autohide(showtime_key, hide_fn, input_areas)
		...
		if state[showtime_key] == nil or hide_timeout < 0 then
			return
		end
